...
| Maccy | App | `cask "maccy"` — clipboard manager with searchable history; macOS itself only ever keeps the *latest* clipboard item. Added to the Brewfile ahead of installing it; run `brew bundle --file=Brewfile` to actually install. Afterwards set "launch at login" and the history size in its preferences (manual). Details: setup-docs → `setup-04` |
