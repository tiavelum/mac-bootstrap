...
# Stage 7 is the working environment for collaborating with Claude on the
# setup project. It is deliberately last and skippable: a Mac is fully
# usable without it, and it is not needed to work on unrelated repos.
#
# Usage: ./install.sh [--skip-claude-env]
...
for arg in "$@"; do
  case "$arg" in
    --skip-claude-env) SKIP_CLAUDE_ENV=1 ;;
...
if [ -d "$HOME/vc/macprefs" ]; then
  ...
  echo "      ~/vc/macprefs/macprefs.sh restore"
...
# --- Stage 7: working environment for Claude (optional) -----------------
# Not part of the Mac. This is the transport that publishes commits made
# locally — used by Claude sessions working in these clones, and by the
# macprefs snapshot agent to publish its snapshots. Skip it and everything
# above still works; you then push by hand.
if [ "$SKIP_CLAUDE_ENV" -eq 1 ]; then
  echo "==> [7/7] Working environment — skipped (--skip-claude-env)"
  echo "    Enable later with: ~/vc/git-autosync/install.sh"
else
  echo "==> [7/7] Working environment (optional component)"
